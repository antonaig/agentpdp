...
/** Loose validation: enough to reject garbage without rejecting a slightly extended Product. */
...
export async function askGroq(input: AskInput, deps: AskDeps = {}): Promise<AskResponse> {
  ...
  try {
      ...
      res = await fetchImpl(GROQ_URL, {
        ...
        body: JSON.stringify({
          ...
          messages: [
            ...
            { role: "user", content: `Product JSON:\n${JSON.stringify(compactProduct(input.product))}\n\nQuestion: ${input.question}` },
          ],
        }),
      });
    } catch (err) {
    ...
    if (!answer) throw new AskError(502, "llm_failed", "LLM returned an empty answer");
    return { answer: clampWords(answer, 120), grounded: true, mode: "llm" };
  } finally {
    clearTimeout(timer);
  }
}
